...
export async function GET() {
  ...
  const [forecastResult, { data: closetClothes }, { data: children }] = await Promise.all([
    ...
    supabase
      .from('clothes')
      .select('id, child_name, size, season, image_url')
      .eq('status', 'in_closet'),
...
**Available Clothes in Closet (grouped by child):**
...
- If a child has insufficient items, repeat with the least recently used.
